...
using LinearAlgebra, IntervalMatrices
function LinearAlgebra.generic_normInf(x) # bug fix for inf-norm of Interval vectors: 区間の大小比較でバグりinf-normが計算できない問題があるための一時的な解決方法
    (v, s) = iterate(x)::Tuple
    maxabs = norm(v)
    while true
        y = iterate(x, s)
...
function verifylss_naive(A,b)
    ...
    # C = C_mid .± C_rad
    C = IntervalMatrix(map(Interval,R)) * IntervalMatrix(map(Interval,A))
...
#         err = maximum(abs.(R*r))/(1-α); # Interval arithmetic
        ...
        println("Fail: Matrix may be singular or ill-conditioned.")
        err = nan;
    end
    return x = x̄ .± err.hi
end

function verifylss(A,b) # verify the solution element-wisely
    x̄ = A\b;
    n = length(x̄);
    R = inv(A);
    #########
